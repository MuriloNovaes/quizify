...
@app.post("/quiz/start", response_model=StartQuizResponse, tags=["Quiz"])
def start_quiz(request: StartQuizRequest):
    """
    Endpoint consumido pelo app (`quiz-api.startQuiz`).
    Gera 12 questões (4 fáceis, 4 médias, 4 difíceis) já no formato do frontend,
    na ordem definida pelo `pattern` recebido.
    """
    logger.info(f"Iniciando quiz para o app. pattern={request.pattern} count={request.count}")
    try:
        return quiz_service.start_quiz(pattern=request.pattern)
    except Exception as e:
        logger.error(f"Erro ao iniciar quiz: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@app.post("/quiz/hint", response_model=HintResponse, tags=["IA Assistente"])
def quiz_hint(request: HintRequest):
    """
    Dica server-side para uma questão da tentativa atual (`quiz-api.requestRemoteHint`).
    Usa a questão cacheada no servidor; em falha o app cai no fallback local.
    """
    logger.info(f"Dica solicitada para a questão {request.questionId}.")
    try:
        return quiz_service.get_hint(request.questionId)
    except Exception as e:
